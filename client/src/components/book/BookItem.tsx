...
export default function BookItem({ book, onClick }: BookItemProps) {
  return (
    <Card 
      className={`overflow-hidden transition-shadow hover:shadow-md ${onClick ? 'cursor-pointer' : ''}`} 
      onClick={onClick}
    >
      ...
      <CardContent className="p-3">
        <h3 className="font-medium text-sm text-center line-clamp-1" title={book.title}>
          {book.title}
        </h3>
        <p className="text-xs text-neutral-500 text-center line-clamp-1" title={book.author}>
          {book.author}
        </p>
      </CardContent>
    </Card>
